...
Now with Levels (black/white/gamma), Reset Levels, Invert, and optional WAV export
...
# Replace this with your own base64 PNG if you want a custom icon
_EMBED_PNG_B64 = (
...
class _TapeSynth:
    ...
    def _pulse_train(self, period_ms: float):
        # -1, +1, then rest at 0 until the period completes
...
class App(tk.Tk):
    def __init__(self):
        ...
        set_embedded_icon(self)  # use embedded icon
        ...
        self.canvas.grid(row=0, column=0, rowspan=9, padx=6, pady=6)
...
def run_cli():
    epilog = textwrap.dedent("""
        Examples:
          ...
          # Also emit a WAV (Original only)
          python gal_hres_gui.py --image in.png --out out.gtp --template ORG \\
                                 --wav out.wav --dither "Ordered 8×8" --threshold 132
    """).strip()
